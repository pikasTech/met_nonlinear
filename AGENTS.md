...
- **禁止创建记忆文件**：任何长期约束、排障口径和仓库经验都必须更新到 `AGENTS.md` 或 `docs/reference/*.md`，禁止在 `/memories/` 下新建、维护或依赖记忆文件；若发现已有记忆文件，先迁移到对应长期文档，再删除。
...
参考索引：
- UniDesk microservice 集成：D601 `~/met_nonlinear` 固定通过 UniDesk SSH 透传开发 TypeScript 编排后端、Dockerfile 和训练容器；项目库文件树、结构化详情、`epoch/h` 队列速度和主 server frontend 集成规则详见 [docs/reference/unidesk_microservice.md](docs/reference/unidesk_microservice.md)。
