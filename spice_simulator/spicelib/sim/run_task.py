...
class RunTask(threading.Thread):
    ...
    def run(self):
        ...
        self.print_info(_logger.debug, ": Starting simulation %d: %s" % (self.runno, self.netlist_file))
        # start execution
        try:
        ...
        if self.retcode == 0:
            ...
            if self.raw_file.exists() and self.log_file.exists():
                ...
                self.print_info(_logger.debug, "Simulation Successful. Time elapsed: %s" % sim_time)
                ...
                if self.callback:
                    if self.callback_args is not None:
                        ...
                        callback_print = ''
                    self.print_info(_logger.debug, "Simulation Finished. Calling...{}(rawfile, logfile{})".format(
                    ...
                    try:
                        ...
                        self.stop_time = time.time()
                        self.print_info(_logger.debug, "Callback Finished. Time elapsed: %s" % format_time_difference(
                            self.stop_time - callback_start_time))
                else:
                    self.print_info(_logger.debug, "Simulation Finished. No Callback function given")
            else:
