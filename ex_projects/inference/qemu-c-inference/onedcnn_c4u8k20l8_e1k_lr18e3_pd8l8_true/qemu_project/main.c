...
static port_float debug_post_dense_3[VALIDATION_SEQ_LEN][8u];
static port_float debug_post_dense_4[VALIDATION_SEQ_LEN][8u];
static port_float debug_post_dense_5[VALIDATION_SEQ_LEN][8u];
static port_float debug_post_dense_6[VALIDATION_SEQ_LEN][8u];
static port_float debug_post_dense_7[VALIDATION_SEQ_LEN][8u];
static port_float debug_post_dense_8[VALIDATION_SEQ_LEN][8u];
...
static void clear_debug_buffers(void)
{
    ...
    zero_buffer(&debug_post_dense_3[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_post_dense_4[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_post_dense_5[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_post_dense_6[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_post_dense_7[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_post_dense_8[0u][0u], VALIDATION_SEQ_LEN * 8u);
...
static void run_generated_record(const port_float input_sequence[VALIDATION_SEQ_LEN][CONV_STACK_INPUT_DIM], port_float output_sequence[VALIDATION_SEQ_LEN])
{
    ...
    for (step_index = 0u; step_index < VALIDATION_SEQ_LEN; ++step_index) {
        ...
        for (channel_index = 0u; channel_index < 4u; ++channel_index) {
            ...
            debug_initial_conv[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            ...
            debug_conv_block_0[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        ...
        dense_pointwise_forward(&debug_post_dense_2[step_index][0u], 8u, 8u, &post_dense_3_kernel[0u][0u][0u], &post_dense_3_bias[0u], 1u, &debug_post_dense_3[step_index][0u]);
        dense_pointwise_forward(&debug_post_dense_3[step_index][0u], 8u, 8u, &post_dense_4_kernel[0u][0u][0u], &post_dense_4_bias[0u], 1u, &debug_post_dense_4[step_index][0u]);
        dense_pointwise_forward(&debug_post_dense_4[step_index][0u], 8u, 8u, &post_dense_5_kernel[0u][0u][0u], &post_dense_5_bias[0u], 1u, &debug_post_dense_5[step_index][0u]);
...
int main(void)
{
    ...
    for (record_index = 0u; record_index < VALIDATION_RECORD_COUNT; ++record_index) {
        ...
        uart_puts("validation_post_dense_3_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_post_dense_3[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_post_dense_4_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_post_dense_4[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_post_dense_5_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_post_dense_5[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_post_dense_6_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_post_dense_6[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_post_dense_7_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_post_dense_7[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_post_dense_8_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_post_dense_8[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");
