...
static port_float debug_scaled_input[VALIDATION_SEQ_LEN][RNN_INPUT_DIM];
static port_float debug_rnn_hidden[VALIDATION_SEQ_LEN][RNN_UNITS];
...
static void dense_forward(const port_float input[RNN_UNITS],
                          port_float output[DENSE_UNITS])
{
    ...
    for (out_index = 0u; out_index < DENSE_UNITS; ++out_index) {
        ...
        for (input_index = 0u; input_index < RNN_UNITS; ++input_index) {
            sum += input[input_index] * dense_kernel[input_index][out_index];
        }
        output[out_index] = apply_activation_code(sum, DENSE_ACTIVATION);
    }
}

static port_float output_forward(const port_float input_values[OUTPUT_INPUT_UNITS])
{
    uint32_t input_index;
    port_float sum = output_bias[0u];
    for (input_index = 0u; input_index < OUTPUT_INPUT_UNITS; ++input_index) {
        sum += input_values[input_index] * output_kernel[input_index][0u];
    }
    return apply_activation_code(sum, OUTPUT_ACTIVATION);
}

static void rnn_forward_step(const port_float input_step[RNN_INPUT_DIM],
                             port_float hidden_state[RNN_UNITS],
                             port_float *output_scaled_value,
                             port_float debug_scaled_input_step[RNN_INPUT_DIM],
                             port_float debug_hidden_step[RNN_UNITS],
                             port_float debug_dense_step[DENSE_UNITS],
                             port_float *output_value)
{
    uint32_t unit;
    uint32_t input_index;
    port_float previous_hidden[RNN_UNITS];
    port_float dense_output[DENSE_UNITS];
    port_float scaled_input_step[RNN_INPUT_DIM];
    port_float output_scaled;

    for (input_index = 0u; input_index < RNN_INPUT_DIM; ++input_index) {
    ...
    for (unit = 0u; unit < RNN_UNITS; ++unit) {
        previous_hidden[unit] = hidden_state[unit];
    }

    for (unit = 0u; unit < RNN_UNITS; ++unit) {
        uint32_t hidden_index;
        port_float acc = rnn_bias[unit];

        for (input_index = 0u; input_index < RNN_INPUT_DIM; ++input_index) {
            acc += scaled_input_step[input_index] * rnn_kernel[input_index][unit];
        }

        for (hidden_index = 0u; hidden_index < RNN_UNITS; ++hidden_index) {
            acc += previous_hidden[hidden_index] * rnn_recurrent_kernel[hidden_index][unit];
        }

        hidden_state[unit] = apply_activation_code(acc, RNN_ACTIVATION);
        if (debug_hidden_step != 0) {
            debug_hidden_step[unit] = hidden_state[unit];
        }
    }

    if (HAS_DENSE != 0u) {
        dense_forward(hidden_state, dense_output);
        for (unit = 0u; unit < DENSE_UNITS; ++unit) {
            if (debug_dense_step != 0) {
                debug_dense_step[unit] = dense_output[unit];
            }
        }
        output_scaled = output_forward(dense_output);
    } else {
        if (debug_dense_step != 0) {
            zero_buffer(debug_dense_step, DENSE_UNITS);
        }
        output_scaled = output_forward(hidden_state);
    }
...
static void run_validation_record(const port_float sequence[VALIDATION_SEQ_LEN][RNN_INPUT_DIM],
                                  port_float output_sequence[VALIDATION_SEQ_LEN],
                                  uint32_t reset_state_each_run,
                                  port_float hidden_state[RNN_UNITS],
                                  port_float debug_scaled_input_buffer[VALIDATION_SEQ_LEN][RNN_INPUT_DIM],
                                  port_float debug_rnn_hidden_buffer[VALIDATION_SEQ_LEN][RNN_UNITS],
                                  ...
                                  port_float debug_output_scaled_buffer[VALIDATION_SEQ_LEN])
{
    ...
    if (reset_state_each_run != 0u) {
        zero_buffer(hidden_state, RNN_UNITS);
    }
    ...
    for (step = 0u; step < VALIDATION_SEQ_LEN; ++step) {
        ...
        if (debug_rnn_hidden_buffer != 0) {
            step_hidden = debug_rnn_hidden_buffer[step];
        }
...
int main(void)
{
    ...
    uart_puts("RNN_BENCHMARK_VALIDATION\n");
    ...
    uart_puts("\ninput_dim=");
    uart_put_u32(RNN_INPUT_DIM);
    uart_puts("\nrnn_units=");
    uart_put_u32(RNN_UNITS);
    uart_puts("\ndense_units=");
    uart_put_u32(HAS_DENSE != 0u ? DENSE_UNITS : 0u);
    uart_puts("\ndwt_supported=");
    ...
    for (record_index = 0u; record_index < VALIDATION_RECORD_COUNT; ++record_index) {
        ...
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_scaled_input[0u][0u], VALIDATION_SEQ_LEN, RNN_INPUT_DIM);
        uart_puts("\n");

        uart_puts("validation_rnn_hidden_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_rnn_hidden[0u][0u], VALIDATION_SEQ_LEN, RNN_UNITS);
        uart_puts("\n");
