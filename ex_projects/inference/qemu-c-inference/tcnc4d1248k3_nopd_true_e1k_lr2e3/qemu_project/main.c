...
static port_float debug_initial_projection[VALIDATION_SEQ_LEN][8u];
static port_float debug_tcn_block_1[VALIDATION_SEQ_LEN][8u];
static port_float debug_tcn_block_2[VALIDATION_SEQ_LEN][8u];
static port_float debug_tcn_block_3[VALIDATION_SEQ_LEN][8u];
static port_float debug_tcn_block_4[VALIDATION_SEQ_LEN][8u];
static port_float debug_post_dense_1[VALIDATION_SEQ_LEN][8u];
static port_float debug_output_scaled[VALIDATION_SEQ_LEN][1u];
static port_float debug_tcn_block_1_conv1[VALIDATION_SEQ_LEN][8u];
static port_float debug_tcn_block_2_conv1[VALIDATION_SEQ_LEN][8u];
static port_float debug_tcn_block_3_conv1[VALIDATION_SEQ_LEN][8u];
static port_float debug_tcn_block_4_conv1[VALIDATION_SEQ_LEN][8u];
...
static void clear_debug_buffers(void)
{
    ...
    zero_buffer(&debug_initial_projection[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_tcn_block_1[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_tcn_block_2[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_tcn_block_3[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_tcn_block_4[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_post_dense_1[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_output_scaled[0u][0u], VALIDATION_SEQ_LEN * 1u);
    zero_buffer(&debug_tcn_block_1_conv1[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_tcn_block_2_conv1[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_tcn_block_3_conv1[0u][0u], VALIDATION_SEQ_LEN * 8u);
    zero_buffer(&debug_tcn_block_4_conv1[0u][0u], VALIDATION_SEQ_LEN * 8u);
}

static void run_generated_record(const port_float input_sequence[VALIDATION_SEQ_LEN][TCN_INPUT_DIM], port_float output_sequence[VALIDATION_SEQ_LEN])
{
    uint32_t step_index;
    uint32_t channel_index;
    port_float residual_buffer[8u];
    clear_debug_buffers();
    for (step_index = 0u; step_index < VALIDATION_SEQ_LEN; ++step_index) {
        ...
        dense_pointwise_forward(&debug_input_scaled[step_index][0u], 1u, 8u, &tcn_initial_projection_kernel[0u][0u][0u], &tcn_initial_projection_bias[0u], 1u, &debug_initial_projection[step_index][0u]);
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            port_float raw_value = conv1d_causal_step(&debug_initial_projection[0u][0u], step_index, 8u, 3u, 1u, 8u, &temporal_block_1_conv_1_kernel[0u][0u][0u], &temporal_block_1_conv_1_bias[0u], channel_index);
            debug_tcn_block_1_conv1[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            port_float raw_value = conv1d_causal_step(&debug_tcn_block_1_conv1[0u][0u], step_index, 8u, 3u, 1u, 8u, &temporal_block_1_conv_2_kernel[0u][0u][0u], &temporal_block_1_conv_2_bias[0u], channel_index);
            debug_tcn_block_1[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            debug_tcn_block_1[step_index][channel_index] = apply_activation_code(debug_tcn_block_1[step_index][channel_index], 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            port_float raw_value = conv1d_causal_step(&debug_tcn_block_1[0u][0u], step_index, 8u, 3u, 2u, 8u, &temporal_block_2_conv_1_kernel[0u][0u][0u], &temporal_block_2_conv_1_bias[0u], channel_index);
            debug_tcn_block_2_conv1[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            port_float raw_value = conv1d_causal_step(&debug_tcn_block_2_conv1[0u][0u], step_index, 8u, 3u, 2u, 8u, &temporal_block_2_conv_2_kernel[0u][0u][0u], &temporal_block_2_conv_2_bias[0u], channel_index);
            debug_tcn_block_2[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            debug_tcn_block_2[step_index][channel_index] = apply_activation_code(debug_tcn_block_2[step_index][channel_index], 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            port_float raw_value = conv1d_causal_step(&debug_tcn_block_2[0u][0u], step_index, 8u, 3u, 4u, 8u, &temporal_block_3_conv_1_kernel[0u][0u][0u], &temporal_block_3_conv_1_bias[0u], channel_index);
            debug_tcn_block_3_conv1[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            port_float raw_value = conv1d_causal_step(&debug_tcn_block_3_conv1[0u][0u], step_index, 8u, 3u, 4u, 8u, &temporal_block_3_conv_2_kernel[0u][0u][0u], &temporal_block_3_conv_2_bias[0u], channel_index);
            debug_tcn_block_3[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            debug_tcn_block_3[step_index][channel_index] = apply_activation_code(debug_tcn_block_3[step_index][channel_index], 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            port_float raw_value = conv1d_causal_step(&debug_tcn_block_3[0u][0u], step_index, 8u, 3u, 8u, 8u, &temporal_block_4_conv_1_kernel[0u][0u][0u], &temporal_block_4_conv_1_bias[0u], channel_index);
            debug_tcn_block_4_conv1[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
            port_float raw_value = conv1d_causal_step(&debug_tcn_block_4_conv1[0u][0u], step_index, 8u, 3u, 8u, 8u, &temporal_block_4_conv_2_kernel[0u][0u][0u], &temporal_block_4_conv_2_bias[0u], channel_index);
            debug_tcn_block_4[step_index][channel_index] = apply_activation_code(raw_value, 1u);
        }
        for (channel_index = 0u; channel_index < 8u; ++channel_index) {
...
int main(void)
{
...
#endif
    uart_init();
    uart_puts("TCN_BENCHMARK_VALIDATION\n");
    ...
    for (record_index = 0u; record_index < VALIDATION_RECORD_COUNT; ++record_index) {
        ...
        uart_puts("validation_initial_projection_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_initial_projection[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_tcn_block_1_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_tcn_block_1[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_tcn_block_2_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_tcn_block_2[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_tcn_block_3_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_tcn_block_3[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");

        uart_puts("validation_tcn_block_4_");
        uart_put_u32(record_index);
        uart_puts("=");
        uart_put_matrix_rows(&debug_tcn_block_4[0u][0u], VALIDATION_SEQ_LEN, 8u);
        uart_puts("\n");
