...
#define FRIKAN_LUT_SCALE_LAYER_0 102.40000000f
    ...
    port_float raw_value)
{
...

    {
        ...
        if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
            row = frikan_layer_lut_0[input_index][FRIKAN_LUT_LAST_INDEX];
            ...
            const port_float *row_next = frikan_layer_lut_0[input_index][lower_index + 1u];
            row = frikan_layer_lut_0[input_index][lower_index];
            port_float lower_value_0 = row[0u];
            outputs[0u] += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
            port_float lower_value_1 = row[1u];
            outputs[1u] += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
            port_float lower_value_2 = row[2u];
            outputs[2u] += output_sign * (lower_value_2 + (row_next[2u] - lower_value_2) * frac);
            port_float lower_value_3 = row[3u];
            outputs[3u] += output_sign * (lower_value_3 + (row_next[3u] - lower_value_3) * frac);
            port_float lower_value_4 = row[4u];
            outputs[4u] += output_sign * (lower_value_4 + (row_next[4u] - lower_value_4) * frac);
            port_float lower_value_5 = row[5u];
            outputs[5u] += output_sign * (lower_value_5 + (row_next[5u] - lower_value_5) * frac);
        }
    }
}

#define FRIKAN_LUT_SCALE_LAYER_1 102.40000000f
    ...
    port_float raw_value)
{
...

    {
        ...
        const port_float *row;
        if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
            row = frikan_layer_lut_1[input_index][FRIKAN_LUT_LAST_INDEX];
            outputs[0u] += output_sign * row[0u];
            outputs[1u] += output_sign * row[1u];
            outputs[2u] += output_sign * row[2u];
            outputs[3u] += output_sign * row[3u];
            ...
            port_float frac = mapped_index - (port_float)lower_index;
            const port_float *row_next = frikan_layer_lut_1[input_index][lower_index + 1u];
            row = frikan_layer_lut_1[input_index][lower_index];
            port_float lower_value_0 = row[0u];
            outputs[0u] += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
            port_float lower_value_1 = row[1u];
            outputs[1u] += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
...
#define FRIKAN_LUT_SCALE_LAYER_2 102.40000000f
    ...
    port_float raw_value)
{
...

    {
        ...
        const port_float *row;
        if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
            row = frikan_layer_lut_2[input_index][FRIKAN_LUT_LAST_INDEX];
            outputs[0u] += output_sign * row[0u];
            outputs[1u] += output_sign * row[1u];
            outputs[2u] += output_sign * row[2u];
            outputs[3u] += output_sign * row[3u];
            ...
            port_float frac = mapped_index - (port_float)lower_index;
            const port_float *row_next = frikan_layer_lut_2[input_index][lower_index + 1u];
            row = frikan_layer_lut_2[input_index][lower_index];
            port_float lower_value_0 = row[0u];
            outputs[0u] += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
            port_float lower_value_1 = row[1u];
            outputs[1u] += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
...
#define FRIKAN_LUT_SCALE_LAYER_3 102.40000000f
    ...
    port_float raw_value)
{
...

    {
        ...
        const port_float *row;
        if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
            row = frikan_layer_lut_3[input_index][FRIKAN_LUT_LAST_INDEX];
            outputs[0u] += output_sign * row[0u];
            outputs[1u] += output_sign * row[1u];
            outputs[2u] += output_sign * row[2u];
            outputs[3u] += output_sign * row[3u];
            ...
            port_float frac = mapped_index - (port_float)lower_index;
            const port_float *row_next = frikan_layer_lut_3[input_index][lower_index + 1u];
            row = frikan_layer_lut_3[input_index][lower_index];
            port_float lower_value_0 = row[0u];
            outputs[0u] += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
            port_float lower_value_1 = row[1u];
            outputs[1u] += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
...
#define FRIKAN_LUT_SCALE_LAYER_4 102.40000000f
    ...
    port_float raw_value)
{
...

    {
        ...
        const port_float *row;
        if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
            row = frikan_layer_lut_4[input_index][FRIKAN_LUT_LAST_INDEX];
            outputs[0u] += output_sign * row[0u];
            outputs[1u] += output_sign * row[1u];
            outputs[2u] += output_sign * row[2u];
            outputs[3u] += output_sign * row[3u];
            ...
            port_float frac = mapped_index - (port_float)lower_index;
            const port_float *row_next = frikan_layer_lut_4[input_index][lower_index + 1u];
            row = frikan_layer_lut_4[input_index][lower_index];
            port_float lower_value_0 = row[0u];
            outputs[0u] += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
            port_float lower_value_1 = row[1u];
            outputs[1u] += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
...
#define FRIKAN_LUT_SCALE_LAYER_5 102.40000000f
    ...
    port_float raw_value)
{
...

    {
        ...
        const port_float *row;
        if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
            row = frikan_layer_lut_5[input_index][FRIKAN_LUT_LAST_INDEX];
            outputs[0u] += output_sign * row[0u];
            outputs[1u] += output_sign * row[1u];
            outputs[2u] += output_sign * row[2u];
            outputs[3u] += output_sign * row[3u];
            ...
            port_float frac = mapped_index - (port_float)lower_index;
            const port_float *row_next = frikan_layer_lut_5[input_index][lower_index + 1u];
            row = frikan_layer_lut_5[input_index][lower_index];
            port_float lower_value_0 = row[0u];
            outputs[0u] += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
            port_float lower_value_1 = row[1u];
            outputs[1u] += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
...
#define FRIKAN_LUT_SCALE_LAYER_6 102.40000000f
    ...
    port_float raw_value)
{
...

    {
        ...
        const port_float *row;
        if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
            row = frikan_layer_lut_6[input_index][FRIKAN_LUT_LAST_INDEX];
            outputs[0u] += output_sign * row[0u];
        } else {
            port_float frac = mapped_index - (port_float)lower_index;
            const port_float *row_next = frikan_layer_lut_6[input_index][lower_index + 1u];
            row = frikan_layer_lut_6[input_index][lower_index];
            port_float lower_value_0 = row[0u];
            outputs[0u] += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
        }
    }
...
{
    {
        ...
        port_float layer_0_out_5 = frikan_layer_bias[0u][5u];
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_0[0u][FRIKAN_LUT_LAST_INDEX];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_0[0u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_0[0u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_0_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_0_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
                    port_float lower_value_2 = row[2u];
                    layer_0_out_2 += output_sign * (lower_value_2 + (row_next[2u] - lower_value_2) * frac);
                    port_float lower_value_3 = row[3u];
                    layer_0_out_3 += output_sign * (lower_value_3 + (row_next[3u] - lower_value_3) * frac);
                    port_float lower_value_4 = row[4u];
                    layer_0_out_4 += output_sign * (lower_value_4 + (row_next[4u] - lower_value_4) * frac);
                    port_float lower_value_5 = row[5u];
                    layer_0_out_5 += output_sign * (lower_value_5 + (row_next[5u] - lower_value_5) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_0[1u][FRIKAN_LUT_LAST_INDEX];
                    layer_0_out_0 += output_sign * row[0u];
                    layer_0_out_1 += output_sign * row[1u];
                    layer_0_out_2 += output_sign * row[2u];
                    layer_0_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_0[1u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_0[1u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_0_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_0_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_0[2u][FRIKAN_LUT_LAST_INDEX];
                    layer_0_out_0 += output_sign * row[0u];
                    layer_0_out_1 += output_sign * row[1u];
                    layer_0_out_2 += output_sign * row[2u];
                    layer_0_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_0[2u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_0[2u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_0_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_0_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_0[3u][FRIKAN_LUT_LAST_INDEX];
                    layer_0_out_0 += output_sign * row[0u];
                    layer_0_out_1 += output_sign * row[1u];
                    layer_0_out_2 += output_sign * row[2u];
                    layer_0_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_0[3u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_0[3u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_0_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_0_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_0[4u][FRIKAN_LUT_LAST_INDEX];
                    layer_0_out_0 += output_sign * row[0u];
                    layer_0_out_1 += output_sign * row[1u];
                    layer_0_out_2 += output_sign * row[2u];
                    layer_0_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_0[4u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_0[4u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_0_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_0_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_0[5u][FRIKAN_LUT_LAST_INDEX];
                    layer_0_out_0 += output_sign * row[0u];
                    layer_0_out_1 += output_sign * row[1u];
                    layer_0_out_2 += output_sign * row[2u];
                    layer_0_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_0[5u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_0[5u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_0_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_0_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_0[6u][FRIKAN_LUT_LAST_INDEX];
                    layer_0_out_0 += output_sign * row[0u];
                    layer_0_out_1 += output_sign * row[1u];
                    layer_0_out_2 += output_sign * row[2u];
                    layer_0_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_0[6u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_0[6u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_0_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_0_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_0[7u][FRIKAN_LUT_LAST_INDEX];
                    layer_0_out_0 += output_sign * row[0u];
                    layer_0_out_1 += output_sign * row[1u];
                    layer_0_out_2 += output_sign * row[2u];
                    layer_0_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_0[7u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_0[7u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_0_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_0_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
    ...
    }
    {
        ...
        port_float layer_1_out_5 = frikan_layer_bias[1u][5u];
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_1[0u][FRIKAN_LUT_LAST_INDEX];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_1[0u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_1[0u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_1_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_1_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
                    port_float lower_value_2 = row[2u];
                    layer_1_out_2 += output_sign * (lower_value_2 + (row_next[2u] - lower_value_2) * frac);
                    port_float lower_value_3 = row[3u];
                    layer_1_out_3 += output_sign * (lower_value_3 + (row_next[3u] - lower_value_3) * frac);
                    port_float lower_value_4 = row[4u];
                    layer_1_out_4 += output_sign * (lower_value_4 + (row_next[4u] - lower_value_4) * frac);
                    port_float lower_value_5 = row[5u];
                    layer_1_out_5 += output_sign * (lower_value_5 + (row_next[5u] - lower_value_5) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_1[1u][FRIKAN_LUT_LAST_INDEX];
                    layer_1_out_0 += output_sign * row[0u];
                    layer_1_out_1 += output_sign * row[1u];
                    layer_1_out_2 += output_sign * row[2u];
                    layer_1_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_1[1u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_1[1u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_1_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_1_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_1[2u][FRIKAN_LUT_LAST_INDEX];
                    layer_1_out_0 += output_sign * row[0u];
                    layer_1_out_1 += output_sign * row[1u];
                    layer_1_out_2 += output_sign * row[2u];
                    layer_1_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_1[2u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_1[2u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_1_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_1_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_1[3u][FRIKAN_LUT_LAST_INDEX];
                    layer_1_out_0 += output_sign * row[0u];
                    layer_1_out_1 += output_sign * row[1u];
                    layer_1_out_2 += output_sign * row[2u];
                    layer_1_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_1[3u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_1[3u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_1_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_1_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_1[4u][FRIKAN_LUT_LAST_INDEX];
                    layer_1_out_0 += output_sign * row[0u];
                    layer_1_out_1 += output_sign * row[1u];
                    layer_1_out_2 += output_sign * row[2u];
                    layer_1_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_1[4u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_1[4u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_1_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_1_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_1[5u][FRIKAN_LUT_LAST_INDEX];
                    layer_1_out_0 += output_sign * row[0u];
                    layer_1_out_1 += output_sign * row[1u];
                    layer_1_out_2 += output_sign * row[2u];
                    layer_1_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_1[5u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_1[5u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_1_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_1_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
    ...
    }
    {
        ...
        port_float layer_2_out_5 = frikan_layer_bias[2u][5u];
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_2[0u][FRIKAN_LUT_LAST_INDEX];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_2[0u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_2[0u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_2_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_2_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
                    port_float lower_value_2 = row[2u];
                    layer_2_out_2 += output_sign * (lower_value_2 + (row_next[2u] - lower_value_2) * frac);
                    port_float lower_value_3 = row[3u];
                    layer_2_out_3 += output_sign * (lower_value_3 + (row_next[3u] - lower_value_3) * frac);
                    port_float lower_value_4 = row[4u];
                    layer_2_out_4 += output_sign * (lower_value_4 + (row_next[4u] - lower_value_4) * frac);
                    port_float lower_value_5 = row[5u];
                    layer_2_out_5 += output_sign * (lower_value_5 + (row_next[5u] - lower_value_5) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_2[1u][FRIKAN_LUT_LAST_INDEX];
                    layer_2_out_0 += output_sign * row[0u];
                    layer_2_out_1 += output_sign * row[1u];
                    layer_2_out_2 += output_sign * row[2u];
                    layer_2_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_2[1u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_2[1u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_2_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_2_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_2[2u][FRIKAN_LUT_LAST_INDEX];
                    layer_2_out_0 += output_sign * row[0u];
                    layer_2_out_1 += output_sign * row[1u];
                    layer_2_out_2 += output_sign * row[2u];
                    layer_2_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_2[2u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_2[2u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_2_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_2_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_2[3u][FRIKAN_LUT_LAST_INDEX];
                    layer_2_out_0 += output_sign * row[0u];
                    layer_2_out_1 += output_sign * row[1u];
                    layer_2_out_2 += output_sign * row[2u];
                    layer_2_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_2[3u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_2[3u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_2_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_2_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_2[4u][FRIKAN_LUT_LAST_INDEX];
                    layer_2_out_0 += output_sign * row[0u];
                    layer_2_out_1 += output_sign * row[1u];
                    layer_2_out_2 += output_sign * row[2u];
                    layer_2_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_2[4u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_2[4u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_2_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_2_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_2[5u][FRIKAN_LUT_LAST_INDEX];
                    layer_2_out_0 += output_sign * row[0u];
                    layer_2_out_1 += output_sign * row[1u];
                    layer_2_out_2 += output_sign * row[2u];
                    layer_2_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_2[5u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_2[5u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_2_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_2_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
    ...
    }
    {
        ...
        port_float layer_3_out_5 = frikan_layer_bias[3u][5u];
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_3[0u][FRIKAN_LUT_LAST_INDEX];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_3[0u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_3[0u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_3_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_3_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
                    port_float lower_value_2 = row[2u];
                    layer_3_out_2 += output_sign * (lower_value_2 + (row_next[2u] - lower_value_2) * frac);
                    port_float lower_value_3 = row[3u];
                    layer_3_out_3 += output_sign * (lower_value_3 + (row_next[3u] - lower_value_3) * frac);
                    port_float lower_value_4 = row[4u];
                    layer_3_out_4 += output_sign * (lower_value_4 + (row_next[4u] - lower_value_4) * frac);
                    port_float lower_value_5 = row[5u];
                    layer_3_out_5 += output_sign * (lower_value_5 + (row_next[5u] - lower_value_5) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_3[1u][FRIKAN_LUT_LAST_INDEX];
                    layer_3_out_0 += output_sign * row[0u];
                    layer_3_out_1 += output_sign * row[1u];
                    layer_3_out_2 += output_sign * row[2u];
                    layer_3_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_3[1u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_3[1u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_3_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_3_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_3[2u][FRIKAN_LUT_LAST_INDEX];
                    layer_3_out_0 += output_sign * row[0u];
                    layer_3_out_1 += output_sign * row[1u];
                    layer_3_out_2 += output_sign * row[2u];
                    layer_3_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_3[2u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_3[2u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_3_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_3_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_3[3u][FRIKAN_LUT_LAST_INDEX];
                    layer_3_out_0 += output_sign * row[0u];
                    layer_3_out_1 += output_sign * row[1u];
                    layer_3_out_2 += output_sign * row[2u];
                    layer_3_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_3[3u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_3[3u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_3_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_3_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_3[4u][FRIKAN_LUT_LAST_INDEX];
                    layer_3_out_0 += output_sign * row[0u];
                    layer_3_out_1 += output_sign * row[1u];
                    layer_3_out_2 += output_sign * row[2u];
                    layer_3_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_3[4u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_3[4u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_3_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_3_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_3[5u][FRIKAN_LUT_LAST_INDEX];
                    layer_3_out_0 += output_sign * row[0u];
                    layer_3_out_1 += output_sign * row[1u];
                    layer_3_out_2 += output_sign * row[2u];
                    layer_3_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_3[5u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_3[5u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_3_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_3_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
    ...
    }
    {
        ...
        port_float layer_4_out_5 = frikan_layer_bias[4u][5u];
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_4[0u][FRIKAN_LUT_LAST_INDEX];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_4[0u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_4[0u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_4_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_4_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
                    port_float lower_value_2 = row[2u];
                    layer_4_out_2 += output_sign * (lower_value_2 + (row_next[2u] - lower_value_2) * frac);
                    port_float lower_value_3 = row[3u];
                    layer_4_out_3 += output_sign * (lower_value_3 + (row_next[3u] - lower_value_3) * frac);
                    port_float lower_value_4 = row[4u];
                    layer_4_out_4 += output_sign * (lower_value_4 + (row_next[4u] - lower_value_4) * frac);
                    port_float lower_value_5 = row[5u];
                    layer_4_out_5 += output_sign * (lower_value_5 + (row_next[5u] - lower_value_5) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_4[1u][FRIKAN_LUT_LAST_INDEX];
                    layer_4_out_0 += output_sign * row[0u];
                    layer_4_out_1 += output_sign * row[1u];
                    layer_4_out_2 += output_sign * row[2u];
                    layer_4_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_4[1u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_4[1u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_4_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_4_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_4[2u][FRIKAN_LUT_LAST_INDEX];
                    layer_4_out_0 += output_sign * row[0u];
                    layer_4_out_1 += output_sign * row[1u];
                    layer_4_out_2 += output_sign * row[2u];
                    layer_4_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_4[2u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_4[2u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_4_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_4_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_4[3u][FRIKAN_LUT_LAST_INDEX];
                    layer_4_out_0 += output_sign * row[0u];
                    layer_4_out_1 += output_sign * row[1u];
                    layer_4_out_2 += output_sign * row[2u];
                    layer_4_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_4[3u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_4[3u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_4_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_4_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_4[4u][FRIKAN_LUT_LAST_INDEX];
                    layer_4_out_0 += output_sign * row[0u];
                    layer_4_out_1 += output_sign * row[1u];
                    layer_4_out_2 += output_sign * row[2u];
                    layer_4_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_4[4u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_4[4u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_4_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_4_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_4[5u][FRIKAN_LUT_LAST_INDEX];
                    layer_4_out_0 += output_sign * row[0u];
                    layer_4_out_1 += output_sign * row[1u];
                    layer_4_out_2 += output_sign * row[2u];
                    layer_4_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_4[5u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_4[5u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_4_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_4_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
    ...
    }
    {
        ...
        port_float layer_5_out_5 = frikan_layer_bias[5u][5u];
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_5[0u][FRIKAN_LUT_LAST_INDEX];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_5[0u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_5[0u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_5_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_5_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
                    port_float lower_value_2 = row[2u];
                    layer_5_out_2 += output_sign * (lower_value_2 + (row_next[2u] - lower_value_2) * frac);
                    port_float lower_value_3 = row[3u];
                    layer_5_out_3 += output_sign * (lower_value_3 + (row_next[3u] - lower_value_3) * frac);
                    port_float lower_value_4 = row[4u];
                    layer_5_out_4 += output_sign * (lower_value_4 + (row_next[4u] - lower_value_4) * frac);
                    port_float lower_value_5 = row[5u];
                    layer_5_out_5 += output_sign * (lower_value_5 + (row_next[5u] - lower_value_5) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_5[1u][FRIKAN_LUT_LAST_INDEX];
                    layer_5_out_0 += output_sign * row[0u];
                    layer_5_out_1 += output_sign * row[1u];
                    layer_5_out_2 += output_sign * row[2u];
                    layer_5_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_5[1u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_5[1u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_5_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_5_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_5[2u][FRIKAN_LUT_LAST_INDEX];
                    layer_5_out_0 += output_sign * row[0u];
                    layer_5_out_1 += output_sign * row[1u];
                    layer_5_out_2 += output_sign * row[2u];
                    layer_5_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_5[2u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_5[2u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_5_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_5_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_5[3u][FRIKAN_LUT_LAST_INDEX];
                    layer_5_out_0 += output_sign * row[0u];
                    layer_5_out_1 += output_sign * row[1u];
                    layer_5_out_2 += output_sign * row[2u];
                    layer_5_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_5[3u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_5[3u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_5_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_5_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_5[4u][FRIKAN_LUT_LAST_INDEX];
                    layer_5_out_0 += output_sign * row[0u];
                    layer_5_out_1 += output_sign * row[1u];
                    layer_5_out_2 += output_sign * row[2u];
                    layer_5_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_5[4u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_5[4u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_5_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_5_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
        ...
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_5[5u][FRIKAN_LUT_LAST_INDEX];
                    layer_5_out_0 += output_sign * row[0u];
                    layer_5_out_1 += output_sign * row[1u];
                    layer_5_out_2 += output_sign * row[2u];
                    layer_5_out_3 += output_sign * row[3u];
                    ...
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_5[5u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_5[5u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_5_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                    port_float lower_value_1 = row[1u];
                    layer_5_out_1 += output_sign * (lower_value_1 + (row_next[1u] - lower_value_1) * frac);
    ...
    }
    {
        port_float layer_6_out_0 = frikan_layer_bias[6u][0u];
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_6[0u][FRIKAN_LUT_LAST_INDEX];
                    layer_6_out_0 += output_sign * row[0u];
                } else {
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_6[0u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_6[0u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_6_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_6[1u][FRIKAN_LUT_LAST_INDEX];
                    layer_6_out_0 += output_sign * row[0u];
                } else {
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_6[1u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_6[1u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_6_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_6[2u][FRIKAN_LUT_LAST_INDEX];
                    layer_6_out_0 += output_sign * row[0u];
                } else {
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_6[2u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_6[2u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_6_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_6[3u][FRIKAN_LUT_LAST_INDEX];
                    layer_6_out_0 += output_sign * row[0u];
                } else {
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_6[3u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_6[3u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_6_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_6[4u][FRIKAN_LUT_LAST_INDEX];
                    layer_6_out_0 += output_sign * row[0u];
                } else {
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_6[4u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_6[4u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_6_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                }
            }
        }
        {
            ...
            if (mapped_index <= FRIKAN_LUT_LAST_INDEX_FLOAT) {
                ...
                uint32_t lower_index = (uint32_t)mapped_index;
                if (lower_index >= FRIKAN_LUT_LAST_INDEX) {
                    const port_float *row = frikan_layer_lut_6[5u][FRIKAN_LUT_LAST_INDEX];
                    layer_6_out_0 += output_sign * row[0u];
                } else {
                    port_float frac = mapped_index - (port_float)lower_index;
                    const port_float *row_next = frikan_layer_lut_6[5u][lower_index + 1u];
                    const port_float *row = frikan_layer_lut_6[5u][lower_index];
                    port_float lower_value_0 = row[0u];
                    layer_6_out_0 += output_sign * (lower_value_0 + (row_next[0u] - lower_value_0) * frac);
                }
            }
