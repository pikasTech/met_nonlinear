...
\abstract{电化学地震检波器（MET）凭借高灵敏度、宽频带以及较好的抗倾斜和抗冲击能力，在地球物理勘探中正变得越来越重要，并被认为是下一代地震信号探测的理想方案之一。然而，由于机械、流体与电化学等多个物理过程之间存在复杂耦合，MET 会表现出显著的震级相关非线性，从而引起频率响应漂移并限制动态范围，最终影响地震数据质量和事件检测可靠性。为解决这一问题，本文提出前馈补偿框架 Wiener-KAN。该框架受到 Wiener 结构“线性动态 + 静态非线性”分解思路的启发：在线性部分，利用目标与理想系统间的相对频率响应初始化 Wiener 线性层，以保留频响物理先验；在非线性部分，引入带符号与对称性约束的 Kolmogorov-Arnold 网络（KAN）可训练样条激活函数，精准刻画传感器输入输出曲线。这种前馈结构不仅增强了单神经元的非线性表达能力，还避免了传统力反馈方案中的闭环振荡风险。此外，本文首次构建并开放了涵盖幅频耦合效应的三维频率响应数据集，并设计了针对频率特性漂移的幅频损失函数（AFMAE）以提高频域拟合精度。实验结果表明，在输入震级提升超过 \valInputGainExpansion~$\times$ 的条件下，Wiener-KAN 将频率漂移从 \valMainWienerFreqDrift~Hz 降低到 \valPrimaryFreqDrift~Hz，将灵敏度漂移从 \valMainWienerSensDrift\% 降低到 \valPrimarySensDrift\%，并将线性度误差从 \valMainWienerLinearity\% 降低到 \valPrimaryLinearity\%；同时，预计算映射技术使 KAN 激活函数可转换为高效查表，在保持极低误差的同时支持资源受限的嵌入式平台实时运行，在 STM32F405 微控制器上实现了 \valPrimaryKeilFps{} points/s 的处理速率和 \valPrimaryKeilRamKB~KB 的极小 RAM 占用。}
...
基于预测模型的传感器校正方法\cite{WOS:001409548400009}表明，软件前馈补偿能够在保持硬件结构简洁的同时改善传感器输出质量。本文据此提出一种适用于 MET 的板端实时 AI 补偿框架。与力反馈系统相比，该框架通过消除闭环相位裕度问题而具备自然稳定优势，更适用于大动态范围工作场景；同时，其软件化实现方式显著降低了硬件复杂度，更便于便携式和野外应用。
...
综上，本文围绕 MET 非线性补偿中的三项关键挑战——物理一致性、结构可解释性和计算效率——提出 Wiener-KAN：一种将 Wiener 线性层与带符号和对称性约束的 KAN 非线性映射相结合的板端实时 AI 补偿网络，并支持超过 \valInputGainExpansion~$\times$ 的输入震级扩展。
...
本文提出的板端实时 AI 补偿架构如 Fig.~\ref{fig:wiener_kan_framework} 所示，其设计承接前述三支路并联 Wiener 等效模型。并联 Wiener 模型中的每个支路由局部线性动态核 $h_i(s)$ 与静态非线性映射 $f_i(\cdot)$ 组成；Wiener-KAN 将这一分解进一步神经网络化，用 Wiener 线性层承载幅值条件局部线性动态，用 KAN 将静态非线性映射 $f_i(\cdot)$ 扩展为可训练的样条函数组合。与传统力反馈方案相比，这种前馈结构不需要额外机械反馈装置，因此能够避免力反馈系统中由非线性引起的闭环振荡问题，从而支持更宽震级范围和频带范围下的运行。
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\columnwidth]{../figures/fig_23_kan_neuron_compensation_montage.png}
...
\subsection{LUT-Based KAN Inference for Embedded Deployment}

KAN 的在线推理代价主要来自每条边上的可学习激活函数求值。对于第 $l$ 层中连接输入 $i$ 与输出 $j$ 的激活函数 $\phi_{l,j,i}(x)$，精确路径需要在每个采样点计算 B 样条基函数和系数加权和。本文在训练完成后将该激活函数离线采样为均匀查找表，使在线推理阶段把样条求值转换为地址映射、读表以及可选的一阶线性插值。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.78\columnwidth]{../figures/fig_15_lut_lookup_principles.png}
...
\begin{figure*}[!t]
    ...
    \caption{Effectiveness of the unified Wiener-KAN main model in reducing amplitude-dependent frequency-response drift. (a) Natural-frequency trajectory versus magnitude before and after compensation. (b) Sensitivity trajectory at \valSensitivityCompareFreq~Hz before and after compensation. (c) Frequency-drift and sensitivity-drift values before and after compensation. (d) Frequency-response curves before and after compensation under representative magnitude conditions. (e) Representative time-domain outputs under selected frequency and magnitude conditions.}
    \label{fig:frequency_response_comparison}
\end{figure*}

Fig.~\ref{fig:frequency_response_comparison}(a)--(b) 展示了 Wiener-KAN 对幅值相关频率响应漂移的抑制效果。补偿后，不同震级下的固有频率轨迹和 \valSensitivityCompareFreq~Hz 灵敏度轨迹向低幅值参考响应收敛。Fig.~\ref{fig:frequency_response_comparison}(c) 给出频率漂移和灵敏度漂移的总体下降幅度，Fig.~\ref{fig:frequency_response_comparison}(d) 展示补偿前后的幅频响应曲线，显示不同震级条件下的响应包络明显收束。Fig.~\ref{fig:frequency_response_comparison}(e) 给出了在 \valFreqScanStart~Hz 到 \valFreqScanEnd~Hz 范围内、不同震级条件下的代表性时域波形。预测波形在相位和幅值上与理想参考输出保持一致，说明频域漂移抑制同时对应到时域补偿质量改善。
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{../figures/fig_05_onboard_inference.png}
...
Tab.~\ref{tab:models} 在统一 MET 补偿协议下比较了 Wiener-KAN 与典型时序基线。Wiener-KAN 同时取得最低频率漂移、最低灵敏度漂移和最高板端吞吐率；TCN 在线性度误差上接近 Wiener-KAN，但其 KEIL speed 明显低于主模型；RNN 的板端吞吐率较高，但物理补偿指标落后于 Wiener-KAN。

\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.98\textwidth]{../figures/fig_02_horizontal_summary.png}
...
Struct 分组结果表明，两类替换分别对应 Wiener-KAN 的线性动态骨架和非线性补偿主体。CNN-KAN 在频率漂移与灵敏度漂移上接近基线，同时在线性度误差上出现更明显的退化，说明局部频率响应初始化的 Wiener 前端更有利于保持带内补偿曲线的稳定形状。Wiener-MLP 在三项物理指标上均明显落后于 Wiener-KAN，说明共享 Wiener 前端后，带物理约束的 KAN 映射仍然承担了幅值相关非线性补偿的主要表达能力。


\subsubsection{Physical sign and symmetry constraints}

约束消融实验将奇对称性与正值性分开考察。奇对称性对应 MET 对正负振动输入的方向一致响应，正值性对应正输入区间内的单调方向保持。各约束变体共享相同 Wiener 前端、样条阶数、网格数量和训练数据，使 Tab.~\ref{tab:ablation_overview} 中的约束分组直接反映结构约束本身对频率漂移、灵敏度漂移和线性度误差的影响。Tab.~\ref{tab:ablation_overview} 中的 \textit{Odd / No positive basis} 对照保留奇对称性，但移除了正输入区间的方向保持约束，因此其优化过程更容易偏离稳定补偿映射。该对照在频率漂移、灵敏度漂移和线性度误差上同时明显恶化，说明正值约束为 Wiener-KAN 提供了稳定的函数形态先验。Baseline 同时保留奇对称性与正值性，使非线性补偿映射更符合器件响应规律，因此能够在三项物理指标上保持更稳定的收敛结果。

\subsubsection{Wiener front-end initialization and trainability}

Tab.~\ref{tab:ablation_overview} 中的 Wiener front 分组评价 Wiener 前端是否受益于局部频率响应初始化，以及注入后的 RNN-style IIR 层是否适合训练。冻结的系统先验设置给出最好的频率漂移、灵敏度漂移和线性度误差。随机初始化且冻结的前端仍可训练，但三项物理指标均退化。可训练 IIR 前端放大拟合频率漂移指标，说明局部频率响应先验作为稳定线性动态骨架时最有效。
