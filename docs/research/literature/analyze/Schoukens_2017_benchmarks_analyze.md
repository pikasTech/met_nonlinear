...
10. **Bouc-Wen模型方程** (第248行公式(1)/第251行描述):
    > 第248行公式(1): ${m}_{L}\ddot{y}\left( t\right)  + r\left( t\right)  + z\left( t\right)  = u\left( t\right)$
    > 第251行[EN]: "where m_L is the mass, y the output position, u the input force, r the linear restoring force and z the nonlinear force which models the hysteretic memory of the system."
...
- 第33行[EN]: 机器人技术和机械工程中基于微分方程的动态模型背景
- 第37行[EN]: 论文研究目的（"The aim of this paper is to evaluate if the continuous-time output error method is suitable for identifying..."）
- 第53行[EN]: OEM方法原理（"the unknown system parameters are tuned so that the simulated model output fits the measured system output"）
- 第57行[EN]: 连续时间系统仿真（"To simulate the continuous-time system and obtain a simulated output, the differential equations must be solved"）
- 第105行[EN]: 级联水箱模型源自伯努利原理
- 第205行[EN]: OEM适用于级联水箱的原因（非线性、导数不连续）
- 第213行[EN]: 级联水箱使用Simulink建模，ode45求解
- 第231行[EN]: OEM处理非线性模型的结论
- 第243行[EN]: Bouc-Wen系统是机械工程中表示滞回效应的单自由度振荡器
- 第248行[EN]/第251行[EN]: Bouc-Wen模型方程（公式(1)及参数描述）