...
**论文证明了什么**：
- KAN比MLP慢约10倍（原文第95行[EN]）："KAN variants typically suffer from 10× slower runtimes than MLPs with comparable model and parameter sizes"
- LUT插值可有效替代运行时函数调用（原文第317-319行[EN]）："Lookup Table (LUT) with Interpolation... eliminating expensive trigonometric evaluations or recurrence formulations"
- GPU优化技术可显著提升KAN效率（原文第69行[EN]）："PolyKAN delivers 1.2-10× faster inference and 1.4-12× faster training"
- KAN已成功扩展用于重构各种神经网络模块（原文第91行[EN]）："KANs have been successfully extended to reconstruct various neural network modules, including convolutional, graph architectures, even Transformer and large language models"
- PolyKAN是首个KAN开源通用GPU算子库（原文第15行[CN]）："PolyKAN which is the first general open-source implementation of KAN and its variants"
- Chebyshev KAN利用三角恒等式避免重复sin/cos求值（原文第169行[EN]）："exploiting the trigonometric identity... propagating between successive orders without invoking sin(kx) or cos(kx) for every k"
- 2D Tiling改善数据访问空间局部性（原文第321-323行[EN]）："improves data access spatial locality, which enhances cache reuse and enables fine-grained parallelism"
- Two-Stage Reduction减少原子争用（原文第325-327行[EN]）："reducing atomic contention and write-conflict overhead"
- Coefficient Layout Reordering实现连续内存访问（原文第329-331行[EN]）："enabling contiguous memory accesses and higher bandwidth utilization"
- PolyKAN在消费级GPU和高端GPU上精度相同（原文第69行[EN]）："with identical accuracy on speech, audio-enhancement, and tabular-regression workloads on both highend GPU and consumer-grade GPU"
...
| 第69行[EN] | PolyKAN性能提升：1.2-10×推理加速，1.4-12×训练加速 |
| 第91行[EN] | KAN已成功扩展用于重构各种神经网络模块 |
| 第95行[EN] | KAN效率问题：比MLP慢约10倍 |
| 第169行[EN] | Chebyshev多项式利用三角恒等式避免重复sin/cos求值 |
| 第317-319行[EN] | LUT插值方法：离线预计算+运行时线性插值 |
| 第321-323行[EN] | 2D Tiling：改善数据访问空间局部性，增强缓存重用 |
| 第325-327行[EN] | Two-Stage Reduction：减少原子争用和写冲突开销 |
| 第329-331行[EN] | Coefficient Layout Reordering：实现连续内存访问和更高带宽利用率 |
| 第341-343行[EN/CN] | 多项式基函数的通用属性 |
