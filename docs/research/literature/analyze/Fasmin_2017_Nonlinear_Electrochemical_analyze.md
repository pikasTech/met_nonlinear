...
3. **非线性阻抗的幅度依赖性**：论文中的极化电阻表达式${\left( {R}_{p,{NL}}\right) ^{-1} = {i}_{\text{ corr }}\left\lbrack  {\left( {{b}_{a} + {b}_{c}}\right)  + \frac{\left( {b}_{a}^{3} + {b}_{c}^{3}\right) }{8}{E}_{ac0}^{2}} ...\right\rbrack$（第269-275行）表明非线性阻抗是扰动幅度E_ac0的函数，而非温度的函数。这种扰动幅度依赖性是Wiener/Volterra模型能够有效描述的。
