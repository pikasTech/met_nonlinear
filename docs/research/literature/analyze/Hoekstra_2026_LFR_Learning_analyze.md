...
**论文做了什么**:
- 第33行[EN]/第35行[CN]: 指出FP模型"仅提供近似的系统描述"，需要通过学习组件增强
...
| 编号 | 引用位置 | 内容摘要 | 状态 |
|------|---------|---------|------|
| 1 | 第33行[EN] | FP模型"仅提供近似的系统描述"，需要通过学习组件增强 | ✅ |
| 2 | 第69-71行[EN] | 物理信息神经网络将物理先验以方程形式嵌入代价函数 | ✅ |
| 3 | 第73-75行[EN] | 模型增强方法：将基线模型与灵活函数逼近器(如ANN)结合 | ✅ |
| 4 | 第81-83行[EN] | LFR的模块化和灵活性允许系统化模型增强 | ✅ |
| 5 | 第167行[EN] | 基线模型与学习组件在组合模型结构中相结合 | ✅ |
| 6 | 第172行[EN] | 通用模型增强结构公式(3a): x_{b,k+1} = (f_base ⋆ f_aug)(...) | ✅ |
| 7 | 第183行[EN] | 算子⋆表示两个函数之间的互连（静态并行、静态串联等） | ✅ |
| 8 | 第187-188行[EN] | 需要对算子⋆进行参数化以实现通用增强结构 | ✅ |
| 9 | 第215行[EN] | 在本节中，我们在基于LFR的增强结构中对(3)进行通用表示 | ✅ (新增) |
| 10 | 第223行[EN] | 提出基于LFR的统一结构，可以表示所有的增强安排 | ✅ (新增) |
