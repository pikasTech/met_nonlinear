...
3. **SHAP从分类任务向频率补偿场景迁移的核心障碍——谐波交互复杂性**：
   ...
   **3.3 频率响应函数本身的结构可解释性 vs 输入特征重要性**：
   ...
   - 这是**完全不同的分析维度**：
      - 分类场景（KAN固有可解释性）：可解释性 = KAN组合图边权重分析，$x_5, $x_{10}, $x_{12}对应边权重较粗（设计阶段分析）
      - 分类场景（SHAP事后分析）：可解释性 = SHAP特征贡献排序，$x_{10}, $x_{12}是主要贡献，$x_5$不在前列（原文第677行）
      - 频率补偿场景：可解释性 = 频响函数的B样条曲线是否逼近真实系统$H(\omega)
    - **SHAP特征重要性排序的量化数据**（原文第677行）：SHAP分析发现$x_{10}, $x_{12}是主要贡献特征，$x_9$对分类贡献小，而$x_5$不在SHAP top contributors之列。KAN图结构分析中$x_5$边权重粗不代表其在SHAP分析中重要——两者是不同维度的可解释性方法。
