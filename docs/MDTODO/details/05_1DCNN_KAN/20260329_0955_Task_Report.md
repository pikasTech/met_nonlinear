# R1.1.2 任务报告：CNNKAN 实施细则计划（综合 R1.1 + R1.1.1）

**任务**: 综合 R1.1 和 R1.1.1，编写新的实施细则计划文件，详细计划每个代码修改点  
**参考模型**: FRIKANh8u6l6  
**日期**: 2026-03-29

---

## 1. 综合方案概述

### 1.1 核心思路
综合 R1.1（方案2详细代码）和 R1.1.1（LSTM 简洁风格），形成最终实施方案：
- **模型风格**: 参考 LSTM，不依赖 system 参数，直接实例化
- **滤波器替换**: 用 Conv1D 完全替换 IIR
- **KAN 结构**: 保持不变

### 1.2 模型架构
```
Input(1) → Conv1D(filters=8, kernel_size=3) → DenseKAN×6 → DenseKAN → Output(1)
              ↓
         h8=8通道输出（CNN输出通道数）
```

### 1.3 与原方案的关键差异

| 特性 | R1.1 方案 | R1.1.1 LSTM风格 | 最终方案 R1.1.2 |
|------|-----------|-----------------|-----------------|
| 参数接口 | `fromSystem()` 兼容接口 | 直接 `__init__` | 直接 `__init__` |
| system 参数 | `iir_params_list=None` | 不需要 | 不需要 |
| model_engine 适配 | 复用 FRIKAN 逻辑 | 需要新 case | 需要新 case |

---

## 2. 文件修改清单

### 2.1 需要创建的文件

| 序号 | 文件路径 | 说明 |
|------|----------|------|
| 1 | `projects/CNNKANh8u6l6/config.json` | 项目配置 |
| 2 | `projects/CNNKANh8u6l6/data/` | 数据目录（空目录） |

### 2.2 需要修改的文件

| 序号 | 文件路径 | 修改内容 |
|------|----------|----------|
| 1 | `src/models/frikan_models.py` | 添加 `CNNKAN` 类（约 200 行） |
| 2 | `src/models/__init__.py` | 导出 `CNNKAN` |
| 3 | `src/core/model_engine.py` | 添加 `CNNKAN` 模型处理分支 |

---

## 3. 详细代码修改点

### 3.1 `src/models/frikan_models.py` 添加 CNNKAN 类

在文件末尾（`FRIKAN5` 类之后）添加：

```python
class CNNKAN(BaseModel):
    """
    CNNKAN (1D CNN based KAN) 模型
    
    用 Conv1D 完全替换 IIR 滤波器层，直接继承 BaseModel 构建新模型。
    参考 LSTM 的简洁写法，不需要传入 system 参数。
    
    架构: Input(1) → Conv1D(filters=8, kernel_size=3) → DenseKAN×6 → DenseKAN → Output(1)
    """

    def __init__(self,
                 grid_size=5,
                 grid_range=(-1.0, 1.0),
                 spline_order=3,
                 basis_activation='silu',
                 fs=2000,
                 checkpoint_dir='data',
                 fix_scale_factor=True,
                 disable_basis_activation=True,
                 inner_kan_units=4,
                 inner_kan_layers=1,
                 use_fast_model=True,
                 cnn_filters=8,
                 cnn_kernel_size=3,
                 dropout_rate=0.2,
                 kan_log_grid=False,
                 kan_grid_expand=True,
                 save_each_epoch=False,
                 model_subcfg={}
                 ):
        """
        初始化 CNNKAN 模型

        Args:
            grid_size: KAN网络的网格大小
            grid_range: 网格范围，如(-1.0, 1.0)
            spline_order: 样条阶数
            basis_activation: 基底激活函数
            fs: 采样频率
            checkpoint_dir: 检查点目录
            fix_scale_factor: 是否固定缩放因子
            disable_basis_activation: 是否禁用基底激活
            inner_kan_units: 内部KAN单元数量
            inner_kan_layers: 内部KAN层数
            use_fast_model: 是否使用快速模型
            cnn_filters: Conv1D 输出通道数 (h8 = 8)
            cnn_kernel_size: Conv1D 卷积核大小
            dropout_rate: Dropout比率
            kan_log_grid: 是否使用对数网格
            kan_grid_expand: 是否扩展网格
            save_each_epoch: 是否每个周期保存
            model_subcfg: 模型子配置
        """
        self.model_name = 'CNNKAN'
        self.callback = None
        
        # Conv1D 层替代 IIR
        self.cnn_filters = cnn_filters
        self.cnn_kernel_size = cnn_kernel_size
        self.cnn = tf.keras.layers.Conv1D(
            filters=cnn_filters,
            kernel_size=cnn_kernel_size,
            strides=1,
            padding='same',
            activation='linear',
            name='cnn_filter'
        )
        
        # 快速模型也使用 Conv1D
        self.fast_cnn = tf.keras.layers.Conv1D(
            filters=cnn_filters,
            kernel_size=cnn_kernel_size,
            strides=1,
            padding='same',
            activation='linear',
            name='fast_cnn_filter'
        )
        
        # 输出 KAN 层
        self.kan = DenseKAN(
            units=1,
            grid_size=grid_size,
            grid_range=grid_range,
            spline_order=spline_order,
            use_bias=True,
            basis_activation=basis_activation,
            fix_scale_factor=fix_scale_factor,
            disable_basis_activation=disable_basis_activation,
            kan_log_grid=kan_log_grid,
            grid_expand=kan_grid_expand
        )

        # 内层 KAN 层
        self.kan_inner_layers = [
            DenseKAN(
                units=inner_kan_units,
                grid_size=grid_size,
                grid_range=grid_range,
                spline_order=spline_order,
                use_bias=True,
                basis_activation=basis_activation,
                fix_scale_factor=fix_scale_factor,
                disable_basis_activation=disable_basis_activation,
                kan_log_grid=kan_log_grid,
                grid_expand=kan_grid_expand
            ) for _ in range(inner_kan_layers)
        ]

        self.dropout_rate = dropout_rate
        self.dropout_layer = tf.keras.layers.Dropout(
            self.dropout_rate) if self.dropout_rate > 0.0 else None
        self.fs = fs
        self.features_num = cnn_filters
        self.use_fast_model = use_fast_model
        self.dropout_position = 'input'
        self.save_each_epoch = save_each_epoch
        self.init_checkpoint(checkpoint_dir)
        self.build_model()

    def build_model(self):
        """
        构建并编译模型
        """
        # 输入层
        input_layer = tf.keras.layers.Input(shape=(None, 1), name='input')
        if self.dropout_layer is not None and self.dropout_position == 'input':
            input_drop_out = self.dropout_layer(input_layer)
        else:
            input_drop_out = input_layer

        # CNN 层
        cnn_out = self.cnn(input_drop_out)
        fast_input = tf.keras.layers.Input(
            shape=(None, self.cnn_filters), name='fast_input')
        fast_cnn_out = self.fast_cnn(fast_input)

        # dropout
        if self.dropout_layer is not None and self.dropout_position == 'cnn':
            cnn_drop_out = self.dropout_layer(cnn_out)
            fast_cnn_drop_out = self.dropout_layer(fast_cnn_out)
        else:
            cnn_drop_out = cnn_out
            fast_cnn_drop_out = fast_cnn_out

        # KAN 内层
        kan_inner_output = self.build_kan_inner_layers(cnn_drop_out)
        fast_kan_inner_output = self.build_kan_inner_layers(fast_cnn_drop_out)

        # dropout
        if self.dropout_layer is not None and self.dropout_position == 'output':
            kan_inner_output = self.dropout_layer(kan_inner_output)
            fast_kan_inner_output = self.dropout_layer(fast_kan_inner_output)

        # 输出层
        output = self.kan(kan_inner_output)
        fast_output = self.kan(fast_kan_inner_output)

        self.model = tf.keras.Model(
            inputs=input_layer, outputs=output, name='CNNKAN')
        self.model.build(input_shape=(None, None, 1))
        
        if self.use_fast_model:
            self.fast_model = tf.keras.Model(
                inputs=fast_input, outputs=fast_output, name='fast_CNNKAN')
            self.fast_model.build(input_shape=(None, None, self.cnn_filters))

    def build_kan_inner_layers(self, cnn_out):
        """
        构建KAN内层

        Args:
            cnn_out: CNN输出

        Returns:
            处理后的张量
        """
        x = cnn_out
        for i, kan_inner in enumerate(self.kan_inner_layers):
            x = kan_inner(x)
            if i + 1 == len(self.kan_inner_layers) // 2:
                if self.dropout_layer is not None and self.dropout_position == 'inner':
                    x = self.dropout_layer(x)
        return x
```

### 3.2 `src/models/__init__.py` 修改

在 `frikan_models` 导入部分添加 `CNNKAN`:

```python
from .frikan_models import (
    FRIKAN, FRIKAND, FRIKAN2, FRIKAN3, FRIKAN4, 
    FRIKAN5, FRIKAN6, FRIKAN23, CNNKAN
)
```

### 3.3 `src/core/model_engine.py` 修改

在 `build_model` 方法中添加 `CNNKAN` 分支（建议添加在 LSTM 分支之后）:

```python
elif self.config.use_model == 'CNNKAN':
    self.model_comp = CNNKAN(
        fs=self.config.sample_rate,
        grid_size=self.config.GRID_SIZE,
        grid_range=(self.config.grid_range[0], self.config.grid_range[1]),
        spline_order=self.config.SPLINE_ORDER,
        basis_activation=self.config.basis_activation,
        fix_scale_factor=self.config.FIX_SCALE_FACTOR,
        inner_kan_units=self.config.INNER_KAN_UNITS,
        inner_kan_layers=self.config.INNER_KAN_LAYERS,
        use_fast_model=self.config.USE_FAST_MODEL,
        checkpoint_dir=self.checkpoint_dir,
        cnn_filters=self.config.CNN_FILTERS,
        cnn_kernel_size=self.config.CNN_KERNEL_SIZE,
        dropout_rate=self.config.dropout_rate if hasattr(self.config, 'dropout_rate') else 0.2,
        kan_log_grid=self.config.kan_log_grid,
        kan_grid_expand=self.config.kan_grid_expand,
        save_each_epoch=self.config.save_each_epoch,
        model_subcfg=self.config.model_subcfg
    )
```

### 3.4 `projects/CNNKANh8u6l6/config.json` 配置文件

```json
{
    "using_gpu": true,
    "epoch_train": 30000,
    "step_per_epoch": 5,
    "learning_rate": 0.01,
    "use_train_model": true,
    "use_model": "CNNKAN",
    "use_power_loss": true,
    "use_points": 8000,
    "resume_training": true,
    "use_best_val_weights": true,
    "SPLINE_ORDER": 2,
    "use_auto_lr": true,
    "auto_lr_decay_steps": 100,
    "USE_FAST_MODEL": true,
    "USE_ASSIGN_WEIGHTS": false,
    "use_scale": true,
    "use_predict_features": true,
    "use_predict_tr": false,
    "use_predict_tr_from_file": false,
    "use_predict_fr": true,
    "use_cache_features": true,
    "use_spline": false,
    "use_sin_fr": false,
    "use_debug_plot": false,
    "sample_rate": 2000,
    "time_clipped_s": 4.0,
    "GRID_SIZE": 8,
    "FIX_SCALE_FACTOR": true,
    "H_UNITS": 8,
    "INNER_KAN_UNITS": 6,
    "INNER_KAN_LAYERS": 6,
    "basis_activation": "tanh",
    "grid_range": [-1, 1],
    "kan_log_grid": false,
    "kan_grid_expand": true,
    "target_sweep": 2,
    "AUG_TIMES": 1,
    "CNN_FILTERS": 8,
    "CNN_KERNEL_SIZE": 3,
    "dropout_rate": 0.2
}
```

---

## 4. 实现步骤

### 步骤1: 创建项目目录结构
```bash
mkdir -p projects/CNNKANh8u6l6/data
```

### 步骤2: 修改 `src/models/frikan_models.py`
- 在文件末尾（FRIKAN5 类之后）添加 `CNNKAN` 类（约 200 行代码）

### 步骤3: 修改 `src/models/__init__.py`
- 在导入列表中添加 `CNNKAN`

### 步骤4: 修改 `src/core/model_engine.py`
- 在 `build_model` 方法中添加 `CNNKAN` 分支

### 步骤5: 创建 `projects/CNNKANh8u6l6/config.json`
- 使用上述配置文件内容

### 步骤6: 验证 CLI 训练功能
```bash
python cli.py --project CNNKANh8u6l6 --mode train
```

### 步骤7: 验证 CLI 评估功能
```bash
python cli.py --project CNNKANh8u6l6 --mode eval
```

---

## 5. 技术细节

### 5.1 Conv1D 层参数
- `filters=8`: 输出通道数，对应 h8
- `kernel_size=3`: 卷积核大小，感受野为3个时间步
- `padding='same'`: 保持时间序列长度不变
- `activation='linear'`: 线性激活（后续 KAN 处理非线性）

### 5.2 与 LSTM 的对比

| 特性 | LSTM | CNNKAN |
|------|------|--------|
| 核心层 | `LSTM(units)` | `Conv1D(filters, kernel_size)` |
| 序列建模 | 递归，长期依赖 | 卷积，有限窗口 |
| 参数来源 | 可学习 | 可学习 |
| system 参数 | 不需要 | 不需要 |
| KAN 层 | 无 | 6 层 DenseKAN |

### 5.3 快速模型 (fast_model)
- 保留 `use_fast_model` 机制
- `fast_cnn` 与 `cnn` 结构相同，训练时直接处理输入序列
- 推理时使用 fast_model 可加速

---

## 6. 配置参数对照表

| 配置项 | FRIKANh8u6l6 | CNNKANh8u6l6 | 说明 |
|--------|---------------|------------------|------|
| `use_model` | FRIKAN | CNNKAN | 模型类型 |
| `H_UNITS` | 8 | 8 | IIR通道数 → 预留 |
| `INNER_KAN_UNITS` | 6 | 6 | 内层KAN单元数 |
| `INNER_KAN_LAYERS` | 6 | 6 | 内层KAN层数 |
| `GRID_SIZE` | 8 | 8 | 样条网格大小 |
| `SPLINE_ORDER` | 2 | 2 | 样条阶数 |
| `basis_activation` | tanh | tanh | 基底激活函数 |
| `CNN_FILTERS` | - | 8 | 新增：Conv1D通道数 |
| `CNN_KERNEL_SIZE` | - | 3 | 新增：卷积核大小 |
| `dropout_rate` | - | 0.2 | 新增：Dropout比率 |

---

## 7. 关键代码修改点汇总

| 文件 | 修改类型 | 修改点 | 代码量 |
|------|----------|--------|--------|
| `src/models/frikan_models.py` | 新增类 | 末尾添加 `CNNKAN` 类 | ~200行 |
| `src/models/__init__.py` | 修改导入 | 添加 `CNNKAN` 到导出列表 | 1行 |
| `src/core/model_engine.py` | 新增分支 | `build_model` 添加 `CNNKAN` 分支 | ~20行 |
| `projects/CNNKANh8u6l6/config.json` | 新建 | 创建项目配置 | ~50行 |
| `projects/CNNKANh8u6l6/data/` | 新建 | 空数据目录 | - |

**总计**: ~270 行新增/修改代码

---

## 8. 风险评估

| 风险 | 影响 | 缓解措施 |
|------|------|----------|
| Conv1D 无法捕获长期依赖 | 中 | KAN 层可补偿部分非线性建模能力 |
| 训练不稳定 | 低 | 从小学习率开始，使用 `use_auto_lr` |
| 与 model_engine 集成问题 | 低 | 遵循 LSTM 模式，添加专门分支 |

---

## 9. 结论

R1.1.2 综合计划已完成。核心要点：
1. CNNKAN 遵循 LSTM 风格，不依赖 system 参数
2. 用 `Conv1D` 替换 `IIR` 滤波器层
3. 保持 `KAN` 网络结构不变
4. 在 `model_engine.py` 中添加专门分支处理
5. 实现工作量约 270 行代码

下一步可进入 R2 实现阶段。
