...
| 部署一致性层 | `QEMU-MAE`、`KEIL-MAE`、`KEIL-FPS` | C 推理与 TensorFlow 参考路径的一致性及板端速度展示 | 部署 EP 产物 |
...
6. 论文正式表格统一从 `metrics.json` 读取，推荐优先读取 `display_metrics`；若同时存在 `KEIL-SPEED` 与 `KEIL-FPS`，论文正文和图表优先使用 `KEIL-FPS (Points/s)`。
...
`metrics_summary.py` 当前重新采用 `fit_params_comped` 作为正式口径，但不再直接信任未经约束的拟合中心频率，而是先将其限制在 band 内。对第 $k$ 个震级的拟合参数 $(A_k, B_k, C_k)$，中心频率定义为：
...
这里的 `clip` 与 `10-128 Hz` band 对应当前论文正式口径；在实现层面，这既体现在 `metrics_summary.py` 的 summary 重算上，也体现在 `model_analysis.py` 生成 `linear_response.json` 时对 `ws_system_fit(...)` 的中心频率约束上。随后用中位数中心漂移定义：
...
\max
\left(
\left|\max_k f_{\mathrm{center},k} - \operatorname{median}(f_{\mathrm{center}})\right|,
\left|\operatorname{median}(f_{\mathrm{center}}) - \min_k f_{\mathrm{center},k}\right|
\right)
$$

因此，`Freq Drift (Hz)` 的实际语义是“跨震级 compensated、band-limited fitted center frequency 漂移”。它仍然保留了原始二阶拟合的物理解释，但通过 band limit 避免异常 project 把中心频率抛出物理频带之外，从而生成明显失真的爆炸漂移数值。
...
该默认值来自 `compare/compute_cost_calibration` 对 STM32F405 真机时延的反标结果：先固定 `add_weight = 1.0`，再搜索 `mul_weight` 与 `map_weight`，并通过整体缩放系数拟合板端 `ms/point`。因此，当前 `Compute Cost` 的语义仍然是单样本、单时间步的加权语义操作估算，而不是完整序列的端到端实测时延；只是其默认平台权重已经从早期的启发式 `1:1:6` 更新为经真机标定后的 `1:3:20`。
...
论文正文与图表的人类可读速度展示则进一步定义为：

$$
F_{\mathrm{KEIL}}
=
\frac{1000}{S_{\mathrm{KEIL}}}
$$
...
- `keil_benchmark_summary.json.keil_speed_points_per_second` 或训练 project summary 中的 `board_keil_fps`

因此，论文表格和图中的 `KEIL speed` 应统一显示为 `Points/s`；`ms/point` 只作为低层推导与排障字段保留。Flash、RAM 与 `cycles_per_iter` 当前不自动写入 `metrics.json`，应在部署子章节中单独说明。
