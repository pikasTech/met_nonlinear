...
当前敏感性分析的正式数值统一来自各项目 `data/metrics.json` 的顶层汇总字段，并由 compare 类 `ex_project` 汇总为 paper-ready 产物；论文主表建议至少包含：

- `Freq Drift (Hz)` / `freq_drift_hz`
- `Sens Drift (%)` / `sens_drift_percent`
- `Linearity (%)` / `linearity_percent`
- `status`
...
4. 对 `GRID_SIZE` 和 `SPLINE_ORDER`，应额外强调其主要影响样条离线表达能力，部署表现以导出实现和板端吞吐率为准。

## ex_project 分析入口与产物要求

超参数敏感性分析的后处理应由单独的 compare 类 `ex_project` 承担。当前统一入口为：

- `ex_projects/compare/hparam_sensitivity_r15`

该分析任务通过 `python cli.py ep "compare/hparam_sensitivity_r15"` 执行，至少应完成以下输出：
...
5. 输出一份 paper-ready 的 Markdown / JSON / CSV 产物，供 `values.tex` 或 `docs/paper/data/results.json` 消费。

建议该 `ex_project` 的核心结论至少覆盖：

- 哪些轴已经完整扫描；
- 是否有轴在高侧触碰训练可行域边界，或在更大硬件上解除该边界；
- 基线是否被任何可训练点同时严格支配；
- `GRID_SIZE` 与 `SPLINE_ORDER` 对三项物理指标的影响范围；
- 若只在当前硬件上考虑，哪些配置属于“训练代价较高但无稳定收益”的排除点。
...
- 当前后处理入口：`ex_projects/compare/hparam_sensitivity_r15`
