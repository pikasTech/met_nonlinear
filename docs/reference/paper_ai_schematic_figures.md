...
- 若图中出现正文没有定义的函数符号、变量或缩写，应删除、改成正文已有术语，或在 caption / 相邻正文中补足解释。机制图可以表达趋势和因果链，但不要把未定义符号画成精确模型。
- 对 Fig.1(c) 这类主文机制图，优先保留 `natural frequency`、`sensitivity`、`damping`、`local frequency response` 等正文已有概念；避免新增 `k_eff(A)`、`\eta(A)`、`\zeta(A)` 等没有在主文附近解释的符号。
- 如果用户明确要求“可以模糊一些说明”，图内应使用机制短语和趋势箭头，而不是更多公式化符号。
- 最终验收包括：图像可读、正文位置合理、`python cli.py ep ex_projects/plot/.../<figure_project>` 可重复生成、LaTeX 编译通过。

## 图文一致性审查

AI 生成图必须单独做图文一致性审查，不能只看图像是否美观。审查时按以下问题逐项判断：

1. 图回答的问题是否与正文引入它的位置一致。
2. 图中每个关键标签是否能在正文、caption 或既有术语表中找到对应解释。
3. 图中的因果箭头是否与正文叙述方向一致。
4. 图中是否把趋势性机制误画成精确数值、精确函数或未验证模型。
5. 图中是否重复了主文已有结构图或 supplement 已有流程图，而没有新的信息增量。
6. 图进入主文后，supplement 中是否仍保留了同义重复图或重复说明。

如果审查发现图与正文不一致，优先改图；只有正文确实缺少必要解释时，才补正文。不要为了适配 AI 图而扩张论文中原本没有的理论符号体系。
