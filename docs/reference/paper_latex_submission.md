...
MN 本地稿件统一以 Springer Nature `sn-jnl` 模板为基础。

判定模板已生效的最小标准：

- `main.tex` 使用 `\documentclass[sn-nature]{sn-jnl}`。
- 本地编译日志中实际加载 `sn-jnl.cls`。
- 能生成 `docs/paper/latex/build/main.pdf`。

仅仅拷入模板文件但未参与编译，不算模板已生效。
